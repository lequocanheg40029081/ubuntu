<table class="table table-striped jambo_table bulk_action">
    ...
    <tbody>
        ...
        <tr class="even<?= $val['id'] ?> pointer">
            ...
            <td><input type="checkbox" onclick="checkPublish(<?= $val['id'] ?>,'publish')" id="publish<?= $val['id'] ?>"
                    data-control="<?= $data['template'] ?>" <?= $val['publish'] == 1?'checked':'' ?>></td>
...
$(document).ready(function() {
    ...
    $('.pagination li a.page-link').click(function() {
        ...
        callback('cpanel/product/pagination_page', data);
    })
